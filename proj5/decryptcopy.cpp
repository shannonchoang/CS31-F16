...
//checks that two characters don't map to the same character
...
//checks for match starting at the beginning of the word to crib
...
bool match(const char ciphertext[], int pos, const char crib[], char* map) {
	...
	for (;;){
		...
		// if crib is done so we must've matched
		if (crib[cribPos] == '\0') {
			return true;
		}
